...
- **No install toggles beyond `machine_kind` and the optional-package picker.** The kinds (`desktop` / `omarchy` / `devcontainer`) are mostly template context; the picker (`run_before_05-pick-packages.sh.tmpl`, auto-run during apply — never a manual command — recording per-machine choices in `~/.config/dotfiles/package-choices`) is the one sanctioned opt-in. Resist adding further feature flags.
...
**Parity + defossil before every audit.** Before the pre-commit audit, run two end-of-task checks. **Parity:** if the change touches a surface with a macOS ↔ Linux counterpart — keybindings (`hypr/bindings.conf` ↔ `aerospace/aerospace.toml.tmpl`), packages (`overrides:` per manager), shell, WM config — update the mirror side in the *same* task, or state explicitly why it's single-platform (`*_skip`, an Omarchy-only quirk, a Mac-only tool). **Defossil:** do a `/defossil` pass over the diff so no conversation-residue reaches the commit.
...
2. `run_onchange_after_10-packages.sh.tmpl` — installs `.chezmoidata/packages.yaml` `baseline` groups (always) plus `optional` groups (gated per machine by `pkg_chosen` against the picker's choices file), via brew / pacman / yay (AUR) / apt. The per-tool `overrides:` map handles cross-manager name/kind differences (`darwin_kind: cask`, `pacman_aur: …`, `apt_manual: antidote_git`, `devcontainer_skip`). The same script installs `uv` always and the AI CLIs (`claude`/`opencode`/`pi`) when opted in, from upstream — intentional. The before-phase `run_before_05-pick-packages.sh.tmpl` runs every apply and writes the choices file this reads: on first contact it silently adopts already-installed optional apps (or, on a fresh machine with a TTY, prompts through the groups), and afterwards asks only about newly added optional apps; no TTY ⇒ it does nothing.
3. `run_onchange_after_15-mise-install.sh.tmpl` — runs `mise install` whenever `dot_config/mise/config.toml` changes.
4. `run_after_16-upgrade-programs.sh.tmpl` — upgrades all installed programs (brew formulae + casks, mise + runtimes, the `claude`/`opencode`/`pi`/`uv` upstream installers, and pacman/apt/yay on Linux) to latest. Runs on every apply but self-gates to once/week via `~/.cache/dotfiles/upgrade.stamp`; that stamp also drives the weekly "no upgrade in a week" reminder in `dot_zshrc.tmpl`. This is why `chezmoi apply`/`update` keep tools current, not merely present — install scripts (10/15) only ensure presence.
...
6. `run_onchange_after_22-mcp-servers.sh.tmpl` — registers the no-auth MCP servers (playwright + chrome-devtools via `npx`, graphify via `uv tool install`) for both Claude Code (`claude mcp add`) and OpenCode (a `jq` merge into `opencode.json`). Idempotent and best-effort: each tool is only touched if present, `claude mcp add` errors are masked, and the OpenCode merge overwrites the key so it never duplicates. Interactive/OAuth MCPs (Context7) are handled by `scripts/setup-cli-logins`, not here.
7. `run_after_30-claude-settings.sh.tmpl` — reconciles `~/.claude/settings.json` by deep-merging a *declared* desired subset into the live file on every apply (additive only: objects merge recursively with desired winning, arrays union+dedupe, scalars desired-wins — keys not declared are left untouched). Public desired values live in `.chezmoidata/claude-settings.yaml` (`claude_settings`); employer-private `private_plugins` / `private_marketplaces` come from the per-machine `~/.config/chezmoi/chezmoi.toml` `[data]` block, so internal plugin/marketplace names never enter this public repo. Claude Code owns the file at runtime, so we deliberately don't track it as a whole.
...
- **New package:** add to a `baseline` group (installed everywhere) or an `optional` group (opt-in; the picker re-asks on every machine and the shell notice nags until answered). Only add an `overrides:` entry if a manager uses a different name or install kind. The file-hash comment in `run_onchange_after_10-packages.sh.tmpl` re-triggers install on next apply.
