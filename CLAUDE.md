...
- **No install toggles beyond `machine_kind` and the optional-package picker.** The kinds (`desktop` / `omarchy` / `devcontainer`) are mostly template context; `scripts/pick-packages` (per-machine choices in `~/.config/dotfiles/package-choices`) is the one sanctioned opt-in. Resist adding further feature flags.
...
2. `run_onchange_after_10-packages.sh.tmpl` — installs `.chezmoidata/packages.yaml` `baseline` groups (always) plus `optional` groups (gated per machine by `pkg_chosen` against the picker's choices file), via brew / pacman / yay (AUR) / apt. The per-tool `overrides:` map handles cross-manager name/kind differences (`darwin_kind: cask`, `pacman_aur: …`, `apt_manual: antidote_git`, `devcontainer_skip`). The same script installs `uv` always and the AI CLIs (`claude`/`opencode`/`pi`) when opted in, from upstream — intentional. A first-run `run_once_before_05-pick-packages.sh.tmpl` launches the picker (TTY-guarded) so the choices file exists before this runs.
